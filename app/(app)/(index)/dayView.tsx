...
const CalendarDayView: React.FC<CalendarDayViewProps> = ({
		...
		for (let i = 0; i < HOURS_IN_DAY; i++) {
			...
			hours.push(
				<View key={i} className="flex-row" style={{ height: HOUR_HEIGHT }}>
					<View className="items-end justify-start border-r border-gray-300 pr-2" style={{ width: TIME_LABEL_WIDTH }}>
	...
	const processEvents = (events: Event[]): ProcessedEventType[] => {
		...
		regularEvents.forEach((event) => {
			...
			const sortedEvents = concurrentEvents.sort(
				(a, b) => a.start_time.toMillis() - b.start_time.toMillis() || a.event_id.localeCompare(b.event_id) // Secondary sort by ID for consistent ordering - could be changed to event priority
			);
		...
		return (
			<>
				...
				{allDayEvents.length > 0 && (
					<View className="border-b border-gray-300 bg-gray-100 p-2">
						<Text className="mb-1 text-sm font-bold">All-Day Events</Text>
						{allDayEvents.map((event, index) => {
							...
							return (
								<TouchableOpacity
									...
									style={[{ backgroundColor: `${borderColor}20`, borderLeftWidth: 4, borderLeftColor: borderColor }]}
									className="mr-1 rounded-md px-2 py-1"
									onPress={() => onEventPress && onEventPress(event)}
								>
									<Text className="text-xs font-bold">{event.name}</Text>
								</TouchableOpacity>
							);
						})}
					</View>
				...
				{regularEvents.map((event, index) => {
					...
					return (
						<TouchableOpacity
							...
							className="mr-1 rounded-r-md px-2 py-1"
							onPress={() => onEventPress && onEventPress(event)}
						>
							<Text className="text-xs font-bold">{event.name}</Text>
	...
	return (
		<View className="flex-1 bg-white">
			<View className="items-center border-b border-gray-200 py-4">
			...
			<ScrollView className="flex-1">
				<View style={{ height: CONTAINER_HEIGHT, position: "relative" }}>
					...
					{showCurrentTime && (
						...
						>
							<View className="absolute -left-1 -top-1 h-2 w-2 rounded-full bg-red-500" />
						</View>
					)}
					<ScrollView className="flex-1">
						<View style={{ height: CONTAINER_HEIGHT, position: "relative" }}>
							...
							<View className="absolute bottom-0 top-0" style={{ left: TIME_LABEL_WIDTH, right: 0 }}>
								{renderEvents()}
							</View>
						</View>
					</ScrollView>
		...
		<View className="flex-1 bg-background">
			<View className="m-2 flex-row items-center justify-between">
				...
				<View className="flex-row items-center">
					<Button onPress={() => setDayBeingViewed(DateTime.now())} className="mx-2 text-primary">
						Today
					</Button>

					<Button onPress={() => router.replace("/weekView")} className="mx-2 text-primary">
						Day
					</Button>
				</View>
			</View>
