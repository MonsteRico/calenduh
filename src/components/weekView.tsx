  ...
  return (
    <div className="flex flex-col">
      <section className="sticky top-16 z-10 flex w-full flex-row">
        <div
          className={`flex w-24 items-end bg-background text-center text-muted-foreground`}
        >
        ...
        <div className="grid-rows-96 grid">
          {fifteenMinChunks.map((time, i) => {
            if (time.minute == 0 && time.hour != 0) {
              return (
                <div
                  className={`relative h-6 w-24 justify-end text-muted-foreground`}
                  key={i}
                >
            ...
            return (
              <div
                className={`h-6 w-24 justify-end text-muted-foreground`}
                key={i}
              ></div>
            );
          })}
  ...
  return (
    <div
      ...
      style={{
        ...
        height: `${
          (event.interval.end.diff(event.interval.start).as("minutes") / 15) *
          1.5
        }rem`,
        width: `${(1 / (event.numConflicts + 1)) * 100}%`,
        left: `${
          numberInConflictingRow > 0
            ? (1 / (event.numConflicts + 1)) * numberInConflictingRow * 100
            : 0
        }%`,
        backgroundColor: rgb
          ? `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, ${eventIsToday ? 0.75 : 0.5})`
          : event.color,
